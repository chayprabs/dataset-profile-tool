...
async def drift_endpoint(
    ...
    with job_workspace() as workspace:
        if before_file:
            ...
            before_profile = profile_dataset(before_path, before_format)
        else:
            prepared_before = prepare_url_source(before_url or "", workspace / "before", before_format)
            before_profile = profile_dataset(prepared_before.path, prepared_before.format)

        if after_file:
            after_path = workspace / (after_file.filename or "after-upload")
            after_path.write_bytes(await after_file.read())
            after_profile = profile_dataset(after_path, after_format)
        else:
            prepared_after = prepare_url_source(after_url or "", workspace / "after", after_format)
            after_profile = profile_dataset(prepared_after.path, prepared_after.format)
